"""Natural Questions document and short-answer collection.

Streams the validation split until ``N_NQ_DOCS`` distinct documents with
usable short answers have been collected. Questions are limited to rows
encountered before that stopping point; this is not an exhaustive collection
of questions for the selected documents.

Document text and answer strings use the same non-HTML tokens and space-join
rule. Recall matching applies lowercase and whitespace normalization to both.
Documents and questions are cached under ``NQ_DIR``.
"""
