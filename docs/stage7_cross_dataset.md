...
Stage 7 asks whether the observed association between chunk size and recall,
and the small matched-size method differences, persist on a second dataset.
Every earlier result (Stages 1-6, including the n=1032 evaluation) was measured
on NQ.
...
unchanged. The check mode below tests whether the fallback is behaviour-identical
for NQ.
...
- Chunking logic and grids: fixed / BiLSTM / Transformer x sizes
  `{6,8,10,12,15}` x overlap `{0,1}` = 30 configs.
...
- Comparability guard: the loader aborts if the median corpus document is
  shorter than 2x the largest grid size (30 sentences). That is the
...
| 2 | method difference unresolved at size 15 | max size-15 spread = 0.0200 | < 2 SE (0.0355) | 0.020 at 15/0 (~1 SE 0.019) |
| 3 | best size is large | best config nominal size = 15 | in {12, 15} | 15 at both NQ scales |
...
2. The nominal best method changed. On NQ the best config was fixed 15/0; on
   TriviaQA it is BiLSTM t15/1, by a margin (0.020) inside the approximate
   2 SE band (0.036). This evaluation does not resolve a method difference.
