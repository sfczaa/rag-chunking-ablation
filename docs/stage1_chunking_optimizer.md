...
- Sweep (`rag_chunk/sweep.py`): builds an in-memory FAISS index per config,
  ...
  are discarded after scoring (nothing is cached under `nq/` unless index saving is enabled).
...
| `recall_vs_chunk_size.png` | doc-constrained Recall@5 vs avg chunk size, one line per (method x overlap) |
