...
- Fixed / BiLSTM / Transformer sweep grids: sizes `{6,8,10,12,15}` x
...
- `rerank_sweep_results.csv` - one row per (arm x method x chunk config);
  30 configs x 3 arms = 90 rows on the full grid. Includes
...
1. Reranking the top-20 raised mean R@1 by 0.044 (~9 of 203 questions), with
   positive deltas in 26/30 configs. The configuration counts are descriptive:
...
2. The gain concentrates at small chunks and vanishes at the sweet spot.
   ...
   ceiling.
3. A deeper pool was worse in this sweep. rerank50 did not beat rerank20 on R@1
