...
- `hybrid_sweep_results.csv` - one row per (retriever x method x chunk config);
  30 configs x 3 retrievers = 90 rows on the full grid.
...
configurations reuse the same questions. All three retrievers improve with
chunk size in this run.
