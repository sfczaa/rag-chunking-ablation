...
- Dataset source (NQ validation stream), chunking logic, fixed / BiLSTM /
  Transformer grids: sizes `{6,8,10,12,15}` x overlap `{0,1}` = 30 configs.
...
- Reranker: `BAAI/bge-reranker-base`, off-the-shelf, top-20 only - no
  rerank50 (Stage 5 observed lower top-rank recall for the deeper pool), no fine-tuning.
...
- Check mode compares all 35 rows (30 bge + 5 rerank20) against the
  ...
  prints a verdict). This checks that the Stage 6 code path reproduces the
  Stage 3/5 pipeline before any conclusion is drawn from it.
...
- `stage6_large_eval_results.csv` - one row per (arm x config): 30 bge +
